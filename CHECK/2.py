...
OUTPUT_ROOT = PROJECT_ROOT / "07_classical_ml" / "outputs" / "robust_model"
BINARY_LABELS = {1: 0, 2: 1, 3: 0}  # baseline/amusement = 0, stress = 1
    ...
    for index, label in enumerate(labels):
        ...
        for signal_name, channel_names in channel_map.items():
            ...
            for channel, channel_name in enumerate(channel_names):
                sig_type = SIGNAL_TYPE_MAP[signal_name]
                try:
                    features = extract_full_features(data[:, channel], sig_type, fs=TARGET_FS)
                except Exception:
                    features = extract_full_features(data[:, channel], "acc", fs=TARGET_FS)  # generic fallback
                row.update({f"{signal_name}_{channel_name}_{feature}": value for feature, value in features.items()})
        rows.append(row)
    return pd.DataFrame(rows)


def build_features(subjects: list[str], variant: str) -> pd.DataFrame:
    ...
    for subject in subjects:
        ...
        if variant == "raw":
            ...
            window_result = create_windows_filtered(signals_for_window, fs_dict, labels, TARGET_FS, WINDOW_SEC, OVERLAP)
        elif variant == "filtered":
            filtered_aligned, _ = align_signals(filter_wrist_signals(record["signal"]["wrist"]), record["label"])
            window_result = create_windows_filtered(filtered_aligned, fs_dict, labels, TARGET_FS, WINDOW_SEC, OVERLAP)
        elif variant == "filtered_normalized":
            filtered_aligned, _ = align_signals(filter_wrist_signals(record["signal"]["wrist"]), record["label"])
            normalized_aligned = normalize_per_subject(filtered_aligned)
            window_result = create_windows_normalized(normalized_aligned, fs_dict, labels, TARGET_FS, WINDOW_SEC, OVERLAP)
        else:
            raise ValueError(f"Unknown variant {variant}")
        frames.append(windows_to_features(window_result, subject))
        del record
...
def run_loso(features: pd.DataFrame, model_name: str, variant: str) -> dict:
...
def main() -> None:
    ...
    if not WESAD_ROOT.exists() or not args.subjects:
        raise SystemExit(f"No WESAD subjects found under {WESAD_ROOT}. Point --subjects / Data/Raw/WESAD at the dataset first.")
    ...
    for variant in args.variants:
        features = build_features(args.subjects, variant)
        if features.empty:
        ...
        for model_name in models_to_run:
            ...
            summary = run_loso(features, model_name, variant)
