...
It reads:
    <variant>/loso_fold_results.csv            (per-subject fold metrics)
    <variant>/pooled_classification_report.csv (precision/recall/f1 per class)
    <variant>/features_all_subjects.csv         (raw feature table, for distributions)
...
Usage:
    python plot_analysis.py --root 07_classical_ml/outputs/robust_model \
        --variants raw filtered filtered_normalized --model random_forest
"""
...
def main():
    ...
    parser.add_argument("--root", default="07_classical_ml/outputs/robust_model",
                         help="OUTPUT_ROOT used by train_robust_model.py")
    parser.add_argument("--variants", nargs="+", default=["raw", "filtered", "filtered_normalized"])
    parser.add_argument("--model", default="random_forest",
                         help="Model whose per-fold plots to draw (must match a run that was executed)")
    ...
    for variant in args.variants:
        ...
        fold_csv = variant_dir / "loso_fold_results.csv"
        report_csv = variant_dir / "pooled_classification_report.csv"
        features_csv = variant_dir / "features_all_subjects.csv"

        if fold_csv.exists():
            plot_per_subject_metrics(fold_csv, out_dir, variant, args.model)
        else:
            print(f"[skip] {fold_csv} not found")

        if report_csv.exists():
            plot_classification_report(report_csv, out_dir, variant, args.model)
        else:
    ...
    if summary_csv.exists():
        ...
        print(f"[skip] {summary_csv} not found -- run with --compare-models or multiple variants first")
